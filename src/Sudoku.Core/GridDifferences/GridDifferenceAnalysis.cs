...
/// <summary>
/// Provides a way to analyze difference between grids.
/// </summary>
public static class GridDifferenceAnalysis
{
	/// <summary>
	...
	/// <param name="result">A <see cref="GridDifference"/> result indicating the difference result.</param>
	...
	/// </remarks>
	public static bool TryGetDifference(in Grid left, in Grid right, [NotNullWhen(true)] out GridDifference? result)
	{
		// Determine whether they are totally same.
		if (Unsafe.AreSame(in left, in right) || left == right)
		{
			result = new NothingChangedGridDifference();
			return true;
		}
		...
		if (lr == right)
		{
			result = new ResetGridDifference();
			return true;
		}
		...
		if (lr != rr)
		{
			...
			if ((rightResetGridGivens & leftResetGridGivens) == leftResetGridGivens && leftResetGridGivens != rightResetGridGivens)
			{
				...
				result = new AddGivenGridDifference(givenCandidates, right.Uniqueness != Uniqueness.Bad);
				return true;
			}
			else if ((leftResetGridGivens & rightResetGridGivens) == rightResetGridGivens && leftResetGridGivens != rightResetGridGivens)
			{
				...
				result = new RemoveGivenGridDifference(givenCandidates);
				return true;
			}
			else if (leftResetGridGivens == rightResetGridGivens)
			{
				...
				result = new ChangedGivenGridDifference(changedCandidates);
				return true;
			}
		...
		if ((rightModifiables & leftModifiables) == leftModifiables && leftModifiables != rightModifiables)
		{
			...
			// Check validity of the second puzzle.
			result = new AddModifiableGridDifference(modifiableCandidates, right.Uniqueness != Uniqueness.Bad);
			return true;
		}
		else if ((leftModifiables & rightModifiables) == rightModifiables && leftModifiables != rightModifiables)
		{
			...
			result = new RemoveModifiableGridDifference(modifiableCandidates);
			return true;
		}
		else if (leftModifiables == rightModifiables && !!leftModifiables)
		{
			...
			if (changedCandidates)
			{
				...
				result = new ChangedModifiableGridDifference(changedCandidates);
				return true;
			}
		}
		...
		if (leftEmptyCells == rightEmptyCells)
		{
			...
			{
				result = new AddCandidateGridDifference(addedCandidates, right.Uniqueness != Uniqueness.Bad);
				return true;
			}
			else if (~addedCandidates && removedCandidates && ~changedCandidates)
			{
				result = new RemoveCandidateGridDifference(removedCandidates);
				return true;
			}
			else if (changedCandidates)
			{
				result = new ChangedCandidateGridDifference(changedCandidates);
				return true;
			}
			else
			{
